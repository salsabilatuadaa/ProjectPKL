    ...
    <div class="container-fluid py-4">
        <div class="card">
            ...
            <div class="card-body pt-4 p-3">
                <form action="/update-profile/{{ $krywn->id }}" method="POST" role="form text-left">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="@error('name')border border-danger rounded-3 @enderror">
                                <input class="form-control" value="{{ $krywn->nama }}" type="text" placeholder="Name" id="nama" name="nama" required>
                                    @error('nama')
                                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <div class="@error('email')border border-danger rounded-3 @enderror">
                                <input class="form-control" value="{{ auth()->user()->email }}" style="color: black;" type="email" placeholder="@example.com" id="email" name="email" required >
                                    @error('email')
                                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                            </div>
                        ...
                        <div>
                            ...
                            <div class="@error('lokasi_kerja')border border-danger rounded-3 @enderror">
                                <input class="form-control" value="{{ $krywn->lokasi_kerja }}" style="color: black;" type="lokasi" id="lokasi_kerja" name="lokasi_kerja" required>
                                    @error('lokasi')
                                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                            </div>
